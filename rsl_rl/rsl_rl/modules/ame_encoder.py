...
class AttentionMapEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.latent_proj = nn.Identity() if d_model == output_dim else nn.Linear(d_model, output_dim)
    ...
    def _encode_flat(
        ...
        *,
        ...
        features = self.activation(self.conv1(z.unsqueeze(1)))
        features = self.activation(self.conv2(features))
        features = features.permute(0, 2, 3, 1).reshape(batch_size, length * width, self.d_model - 3)
        point_features = torch.cat((features, xyz.reshape(batch_size, length * width, 3)), dim=-1)
        point_features = self.point_ln(point_features)

        query = self.query_ln(self.proprio_proj(proprio).unsqueeze(1))
        ...
        z_map = self.out_ln(z_map.squeeze(1))
